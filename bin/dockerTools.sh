...
if [ "$contenaire" == "Tous les conteneurs" ]; then
  chemainCompose="./docker-compose.yaml"
  ficherEnv="./.env"
  ...
  if [ "$action" == "logs -f" ]; then
    ...
    docker compose --env-file "$ficherEnv" -f "$chemainCompose" up -d --build
  elif [ "$action" == "stop" ]; then
    docker compose --env-file "$ficherEnv" -f "$chemainCompose" down
  elif [ "$action" == "restart" ]; then
    docker compose --env-file "$ficherEnv" -f "$chemainCompose" down && docker compose --env-file "$ficherEnv" -f "$chemainCompose" up -d --build
